...
@_requires_google
class TestGmailAdapterProtocol:
    """GmailAdapter conforms to PipeAdapter and wraps Gmail API logic."""

    def test_gmail_adapter_conforms(self):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters import PipeAdapter

        assert isinstance(GmailAdapter(), PipeAdapter)

    def test_gmail_adapter_metadata(self):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter

        adapter = GmailAdapter()
        assert adapter.name == "gmail"
        assert adapter.pipe_name == "gmail"
        assert adapter.required_extras == ["google"]

    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", False)
    def test_gmail_adapter_run_missing_google_deps(self):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters import PipeContext, PipeResult, PipeStatus

        db = MagicMock()
        ctx = PipeContext(source_config={"gmail": {"enabled": True}})
        ...
        assert result.status == PipeStatus.SKIPPED
        assert (
            "dependencies" in result.data.get("reason", "").lower() or "google" in result.data.get("reason", "").lower()
        )

    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_gmail_adapter_run_disabled_in_config(self):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters import PipeContext, PipeResult, PipeStatus

        db = MagicMock()
        ctx = PipeContext(source_config={"gmail": {"enabled": False}})
        ...
        assert result.status == PipeStatus.SKIPPED
        assert (
            "enabled" in result.data.get("reason", "").lower() or "not enabled" in result.data.get("reason", "").lower()
        )

    @patch("footprinter.connectors.google.adapters.gmail.emails_db.insert_email")
    @patch("footprinter.connectors.google.adapters.gmail.GmailClient")
    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_gmail_adapter_run_returns_stage_result(self, MockGmailClient, mock_insert):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters import PipeContext, PipeResult, PipeStatus

        entries = [{"subject": "hi"}, {"subject": "bye"}]
        MockGmailClient.return_value.parse_all.return_value = iter(entries)

        db = MagicMock()
        ctx = PipeContext(source_config={"gmail": {"enabled": True, "accounts": []}})
        ...
        assert isinstance(result, PipeResult)
        assert result.status == PipeStatus.COMPLETED
        assert result.data["emails_indexed"] == 2
        assert result.data["errors"] == 0
        assert mock_insert.call_count == 2
    ...
    @patch("footprinter.connectors.google.adapters.gmail.GmailClient")
    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_gmail_adapter_run_with_errors(self, MockGmailClient, mock_insert):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters import PipeContext, PipeStatus

        entries = [{"subject": "a"}, {"subject": "b"}, {"subject": "c"}]
        MockGmailClient.return_value.parse_all.return_value = iter(entries)
    ...
    @patch("footprinter.connectors.google.adapters.gmail.GmailClient")
    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_gmail_adapter_run_total_failure(self, MockGmailClient):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters import ErrorType, PipeContext, PipeStatus

        MockGmailClient.side_effect = Exception("OAuth token expired")
    ...
    def test_gmail_adapter_status(self):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter

        db = MagicMock()
        db.conn.cursor.return_value.fetchone.return_value = [42]

        adapter = GmailAdapter()
        status = adapter.status(db)

        assert status == {"emails": 42}


@_requires_google
class TestGmailAdapterOnProgress:
    """on_progress is passed via PipeContext to GmailAdapter."""

    @patch("footprinter.connectors.google.adapters.gmail.ingest_entries")
    @patch("footprinter.connectors.google.adapters.gmail.GmailClient")
    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_on_progress_passed_to_ingest_entries(self, MockClient, mock_ingest):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters.protocol import PipeContext, PipeResult

        mock_ingest.return_value = PipeResult.completed("gmail")
        cb = lambda n: None
        ctx = PipeContext(
            source_config={"gmail": {"enabled": True, "accounts": []}},
            on_progress=cb,
        )

        GmailAdapter().run(MagicMock(), ctx)

        assert mock_ingest.call_args[1]["on_progress"] is cb

    @patch("footprinter.connectors.google.adapters.gmail.GmailClient")
    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_on_progress_none_by_default(self, MockClient):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters.protocol import PipeContext

        MockClient.return_value.parse_all.return_value = iter([])
        ctx = PipeContext(source_config={"gmail": {"enabled": True, "accounts": []}})
...
@_requires_google
class TestGmailAdapterLastRun:
    """GmailAdapter extracts last_run from PipeContext and passes to GmailClient."""

    @patch("footprinter.connectors.google.adapters.gmail.GmailClient")
    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_adapter_passes_last_run_to_client(self, MockClient):
        ...
        MockClient.assert_called_once_with(ctx.source_config["gmail"], since=cutoff)

    @patch("footprinter.connectors.google.adapters.gmail.GmailClient")
    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_adapter_full_mode_ignores_last_run(self, MockClient):
        from datetime import datetime
        ...
        MockClient.assert_called_once_with(ctx.source_config["gmail"], since=None)

    @patch("footprinter.connectors.google.adapters.gmail.GmailClient")
    @patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True)
    def test_adapter_no_last_run_passes_none(self, MockClient):
        from footprinter.connectors.google.adapters.gmail import GmailAdapter
        from footprinter.ingest.adapters.protocol import PipeContext
