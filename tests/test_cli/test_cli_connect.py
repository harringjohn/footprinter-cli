...
# ---------------------------------------------------------------------------
# RED — source_seeds upsert
# ---------------------------------------------------------------------------


@_requires_google
class TestUpdateConfigSourceSeeds:
    """_update_config_enabled should write source_seeds for drive accounts."""

    def _make_config(self, tmp_path, base=None):
        """Create a temp config file and patch _require_config to use it."""
        import yaml
        config = base or {"google_drive": {"enabled": False}, "gmail": {"enabled": False}}
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump(config))
        return config, config_path

    def test_update_config_adds_source_seeds_for_drive(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled

        config, config_path = self._make_config(tmp_path)

        with patch("footprinter.cli.setup._require_config", return_value=(config, config_path)):
            _update_config_enabled({"personal": ["drive"]}, _google_spec())

        assert "source_seeds" in config
        seeds = config["source_seeds"]
        assert len(seeds) == 1
        assert seeds[0]["source_type"] == "remote"
        assert seeds[0]["account"] == "personal"

    def test_update_config_upserts_existing_source_seeds(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled

        config, config_path = self._make_config(
            tmp_path,
            {
        ...
        seeds = [s for s in config["source_seeds"] if s["source_type"] == "remote"]
        assert len(seeds) == 1, "Should upsert, not duplicate"
        # Existing label is preserved when entry has no explicit label
        assert seeds[0]["label"] == "Old Label"

    def test_update_config_does_not_match_legacy_drive_seed(self, tmp_path):
        """A pre-existing seed with source_type='drive' must NOT be upserted —
        a new 'remote' seed should be appended instead."""
        from footprinter.cli.connect import _update_config_enabled
        ...
        seeds = config["source_seeds"]
        assert len(seeds) == 2, (
            f"Old 'drive' seed should be left alone and a new 'remote' seed appended, got {len(seeds)}: {seeds}"
        )
        assert seeds[0]["source_type"] == "drive", "Old seed should be untouched"
        assert seeds[1]["source_type"] == "remote", "New seed should use 'remote'"

    def test_update_config_skips_seeds_for_gmail_only(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled

        config, config_path = self._make_config(tmp_path)

        with patch("footprinter.cli.setup._require_config", return_value=(config, config_path)):
            _update_config_enabled({"work": ["gmail"]}, _google_spec())

        seeds = config.get("source_seeds", [])
        assert len(seeds) == 0, "Gmail-only should not add source_seeds"

    def test_update_config_populates_gmail_accounts(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled

        config, config_path = self._make_config(tmp_path)

        with patch("footprinter.cli.setup._require_config", return_value=(config, config_path)):
            _update_config_enabled({"personal": ["gmail"]}, _google_spec())

        accounts = config["gmail"].get("accounts", [])
        assert len(accounts) == 1
        assert accounts[0]["name"] == "personal"
        assert "token_path" in accounts[0]

    def test_update_config_upserts_existing_gmail_account(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled
        ...
        config, config_path = self._make_config(
            tmp_path,
            {
                ...
                "gmail": {
                    "enabled": False,
                    "accounts": [
                        {"name": "personal", "token_path": "/old/path/token.json"},
                    ],
                },
            },
        )
        ...
        assert len(accounts) == 1, "Should upsert, not duplicate"
        assert accounts[0]["token_path"] != "/old/path/token.json"

    def test_update_config_skips_gmail_accounts_for_drive_only(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled
        ...
        accounts = config["gmail"].get("accounts", [])
        assert len(accounts) == 0, "Drive-only should not add gmail accounts"

    def test_update_config_populates_both_drive_seeds_and_gmail_accounts(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled

        config, config_path = self._make_config(tmp_path)

        with patch("footprinter.cli.setup._require_config", return_value=(config, config_path)):
            _update_config_enabled({"personal": ["drive", "gmail"]}, _google_spec())
...
# ---------------------------------------------------------------------------
# Config section creation when sections are missing
# ---------------------------------------------------------------------------


@_requires_google
class TestUpdateConfigMissingSections:
    """_update_config_enabled should create sections when config lacks them.

    google_drive/gmail were removed from config.example.yaml. On fresh
    installs neither section exists, so the ``in config`` guards silently skip.
    """

    def _make_config(self, tmp_path, base=None):
        import yaml

        config = base if base is not None else {}
        config_path = tmp_path / "config.yaml"
        config_path.write_text(yaml.dump(config))
        return config, config_path

    def test_update_config_creates_drive_section_when_missing(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled
        ...
        assert "google_drive" in config, "google_drive section should be created"
        assert config["google_drive"]["enabled"] is True

    def test_update_config_creates_gmail_section_when_missing(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled
        ...
        assert "gmail" in config, "gmail section should be created"
        assert config["gmail"]["enabled"] is True
        accounts = config["gmail"].get("accounts", [])
        assert len(accounts) == 1
        assert accounts[0]["name"] == "personal"

    def test_update_config_creates_both_sections_when_missing(self, tmp_path):
        from footprinter.cli.connect import _update_config_enabled
