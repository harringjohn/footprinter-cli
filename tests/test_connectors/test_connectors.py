...
class TestGetConnectorSources:
    ...
    @_requires_google
    def test_includes_installed_connector_adapters(self):
        from unittest.mock import patch
        ...
        # Google connector declares drive_folders, drive_files, gmail
        assert "drive_files" in result
        assert "gmail" in result
        assert "drive_folders" in result
...
@_requires_google
class TestApplyGoogleConfig:
    """Tests for apply_google_config() — shared config mutation logic."""

    def test_noop_on_empty(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {})
        assert config == {}

    def test_enables_drive(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"personal": ["drive"]})
        assert config["google_drive"]["enabled"] is True
        assert "gmail" not in config

    def test_enables_gmail(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"personal": ["gmail"]})
        assert config["gmail"]["enabled"] is True
        assert "google_drive" not in config

    def test_enables_both(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"personal": ["drive", "gmail"]})
        assert config["google_drive"]["enabled"] is True
        assert config["gmail"]["enabled"] is True

    def test_adds_drive_source_seeds(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"personal": ["drive"], "work": ["drive"]})
        seeds = config["source_seeds"]
        assert len(seeds) == 2
        names = {s["account"] for s in seeds}
        assert names == {"personal", "work"}
        for s in seeds:
            assert s["source_type"] == "remote"
            assert s["enabled"] is True

    def test_upserts_existing_drive_seed(self):
        from footprinter.connectors.google.config import apply_google_config
        ...
        config = {"source_seeds": [existing_seed]}
        apply_google_config(config, {"personal": ["drive"]})
        seeds = config["source_seeds"]
        assert len(seeds) == 1
        assert seeds[0]["name"] == "gdrive_personal"
        # Existing label is preserved when entry has no explicit label
        assert seeds[0]["label"] == "Old Label"
        assert seeds[0]["enabled"] is True

    def test_does_not_match_legacy_drive_seed(self):
        from footprinter.connectors.google.config import apply_google_config
        ...
        config = {"source_seeds": [legacy_seed]}
        apply_google_config(config, {"personal": ["drive"]})
        seeds = config["source_seeds"]
        assert len(seeds) == 2
        assert seeds[0]["source_type"] == "drive"
        assert seeds[1]["source_type"] == "remote"

    def test_adds_gmail_accounts(self):
        from footprinter.connectors.google.config import apply_google_config
        ...
        assert "google_personal_token.json" in accounts[0]["token_path"]

    def test_upserts_existing_gmail_account(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {
            "gmail": {
                "enabled": False,
                "accounts": [{"name": "personal", "token_path": "/old/path"}],
            }
        }
        apply_google_config(config, {"personal": ["gmail"]})
        accounts = config["gmail"]["accounts"]
    ...
    def test_no_seeds_for_gmail_only(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"personal": ["gmail"]})
        assert "source_seeds" not in config

    def test_no_gmail_for_drive_only(self):
        from footprinter.connectors.google.config import apply_google_config
    ...
    def test_returns_none(self):
        ...
        result = apply_google_config({}, {"personal": ["drive"]})
        assert result is None

    def test_adds_drive_accounts(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"personal": ["drive"]})
        accounts = config["google_drive"]["accounts"]
        assert len(accounts) == 1
        assert accounts[0]["name"] == "personal"
        assert "google_personal_token.json" in accounts[0]["token_path"]

    def test_upserts_existing_drive_account(self):
        from footprinter.connectors.google.config import apply_google_config
    ...
    def test_drive_accounts_for_multiple_accounts(self):
        from footprinter.connectors.google.config import apply_google_config
    ...
    def test_no_drive_accounts_for_gmail_only(self):
        from footprinter.connectors.google.config import apply_google_config
    ...
    def test_stores_root_folder_id(self):
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"personal": {"services": ["drive"], "root_folder_id": "abc123"}})
        accounts = config["google_drive"]["accounts"]
        assert accounts[0]["root_folder_id"] == "abc123"


@_requires_google
class TestGoogleConnectorPackage:
    def test_google_connector_package_importable(self):
        import footprinter.connectors.google  # noqa: F401


# ---------------------------------------------------------------------------
# RED — register_connector_schema() helper
# ---------------------------------------------------------------------------
...
@_requires_google
class TestApplyGoogleConfigLabel:
    def test_uses_account_label_in_seed(self):
        """Source seed label should use the account's display label, not name."""
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"work": {"services": ["drive"], "label": "Consulting"}})
        seeds = config["source_seeds"]
        assert len(seeds) == 1
        assert seeds[0]["label"] == "Drive (Consulting)"

    def test_preserves_label_in_account_entry(self):
        """Account entry in google_drive.accounts should carry the label field."""
        from footprinter.connectors.google.config import apply_google_config
        ...
        assert accounts[0]["label"] == "Consulting"

    def test_seed_label_defaults_to_name_without_label(self):
        """Without label in entry, seed label falls back to account name."""
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"work": {"services": ["drive"]}})
        seeds = config["source_seeds"]
        assert seeds[0]["label"] == "Drive (work)"

    def test_gmail_account_preserves_label(self):
        """Gmail account entry should also carry the label field."""
        from footprinter.connectors.google.config import apply_google_config

        config = {}
        apply_google_config(config, {"personal": {"services": ["gmail"], "label": "Family"}})
        accounts = config["gmail"]["accounts"]
        assert len(accounts) == 1
        assert accounts[0]["name"] == "personal"
        assert accounts[0]["label"] == "Family"

    def test_rerun_preserves_existing_seed_label(self):
        """Re-running setup without label should not clobber user-set seed label."""
        from footprinter.connectors.google.config import apply_google_config
            ...
            "source_seeds": [
                {
                    ...
                    "source_type": "remote",
                    "account": "work",
                    "label": "Drive (Consulting)",
                    "icon": "cloud",
                    "enabled": True,
                }
            ],
        }
        # Re-run without label (simulates setup wizard)
        apply_google_config(config, {"work": ["drive"]})
        assert config["source_seeds"][0]["label"] == "Drive (Consulting)"

    def test_rerun_preserves_existing_drive_account_label(self):
        """Re-running setup without label should not clobber user-set account label."""
        from footprinter.connectors.google.config import apply_google_config

        config = {
            "google_drive": {
                "enabled": True,
                "accounts": [{"name": "work", "label": "Consulting", "token_path": "/old"}],
            },
        }
        apply_google_config(config, {"work": ["drive"]})
        assert config["google_drive"]["accounts"][0]["label"] == "Consulting"

    def test_rerun_preserves_existing_gmail_account_label(self):
        """Re-running setup without label should not clobber user-set gmail label."""
        from footprinter.connectors.google.config import apply_google_config
        ...
        apply_google_config(config, {"work": ["gmail"]})
        assert config["gmail"]["accounts"][0]["label"] == "Consulting"

    def test_explicit_label_overrides_existing(self):
        """Explicit label in entry should override the existing one."""
        from footprinter.connectors.google.config import apply_google_config

        config = {
            "google_drive": {
                "enabled": True,
                "accounts": [{"name": "work", "label": "Old", "token_path": "/old"}],
            },
            "source_seeds": [
                {
                    "name": "gdrive_work",
        ...
        apply_google_config(config, {"work": {"services": ["drive"], "label": "New"}})
        assert config["google_drive"]["accounts"][0]["label"] == "New"
        assert config["source_seeds"][0]["label"] == "Drive (New)"


@_requires_google
class TestDriveSeedLabel:
    def test_formats_label(self):
        from footprinter.connectors.google.config import drive_seed_label

        assert drive_seed_label("Consulting") == "Drive (Consulting)"

    def test_formats_plain_name(self):
        from footprinter.connectors.google.config import drive_seed_label

        assert drive_seed_label("work") == "Drive (work)"


# ---------------------------------------------------------------------------
# RED — _status_dict() and _print_status_panel() label support
# ---------------------------------------------------------------------------
...
class TestLabelCommand:
    ...
    def test_updates_config(self, tmp_path):
        """Label command should set label on matching account entries and update seed."""
        from unittest.mock import patch
        ...
        args = type("Args", (), {"name": "google", "account": "work", "label": "Consulting"})()
        ...
        # Drive account got the label
        assert config["google_drive"]["accounts"][0]["label"] == "Consulting"
        # Gmail account got the label
        assert config["gmail"]["accounts"][0]["label"] == "Consulting"
        # Source seed label was updated
        assert config["source_seeds"][0]["label"] == "Drive (Consulting)"
...
@_requires_google
class TestSetupSummaryLabel:
    def test_shows_label_when_present(self):
        from io import StringIO
        from unittest.mock import patch

        from rich.console import Console

        from footprinter.cli.google_setup import print_setup_summary

        buf = StringIO()
        test_console = Console(file=buf, force_terminal=False, width=120)

        result = {
            "work": {"services": ["drive", "gmail"], "label": "Consulting"},
        }

        with patch("footprinter.cli.google_setup.console", test_console):
            print_setup_summary(result)

        output = buf.getvalue()
        assert "Consulting" in output

    def test_shows_name_without_label(self):
        from io import StringIO
        from unittest.mock import patch
...
# ---------------------------------------------------------------------------
# RED — Entry-point discovery for Google connector
# ---------------------------------------------------------------------------


# ---------------------------------------------------------------------------
# RED — content_service read_file dispatches via connector hook
# ---------------------------------------------------------------------------
...
class TestEntryPointDiscovery:
    """Verify the Google connector is discoverable via entry points."""

    def test_google_entry_point_registered(self):
        from importlib.metadata import entry_points

        eps = entry_points(group="footprinter.connectors")
        names = [ep.name for ep in eps]
        assert "google" in names

    def test_google_entry_point_loads_connector_spec(self):
        from importlib.metadata import entry_points

        from footprinter.connectors import ConnectorSpec

        eps = entry_points(group="footprinter.connectors")
        google_eps = [ep for ep in eps if ep.name == "google"]
        assert len(google_eps) == 1
        spec = google_eps[0].load()
        assert isinstance(spec, ConnectorSpec)
        assert spec.name == "google"


@_requires_google
class TestGmailAdapterBodyPreviewGating:
    """Gmail adapter must strip body_preview when content_snippets is disabled."""

    def _make_ctx(self, content_snippets=False):
        """Build a PipeContext with gmail enabled and content_snippets flag."""
        from footprinter.ingest.adapters.protocol import PipeContext

        return PipeContext(
            source_config={
                "gmail": {"enabled": True, "accounts": [{"name": "test"}]},
                "indexing": {"content_snippets": content_snippets},
            },
        )

    def _run_adapter_capture_entries(self, content_snippets, fake_emails):
        """Run the adapter and capture the entries passed to ingest_entries."""
        from unittest.mock import MagicMock, patch

        from footprinter.connectors.google.adapters.gmail import GmailAdapter

        adapter = GmailAdapter()
        ctx = self._make_ctx(content_snippets=content_snippets)
        db = MagicMock()

        with (
            patch("footprinter.connectors.google.adapters.gmail._GOOGLE_AVAILABLE", True),
            patch("footprinter.connectors.google.adapters.gmail.GmailClient") as MockClient,
            patch("footprinter.connectors.google.adapters.gmail.ingest_entries") as mock_ingest,
        ):
            mock_instance = MockClient.return_value
            mock_instance.parse_all.return_value = iter(fake_emails)
            mock_ingest.return_value = MagicMock(data={"emails_indexed": 1})

            adapter.run(db, ctx)

            call_args = mock_ingest.call_args
            entries_gen = call_args[0][1]  # second positional arg
            return list(entries_gen)

    def test_body_preview_stripped_when_disabled(self):
        """body_preview is empty when content_snippets is false."""
        entries = self._run_adapter_capture_entries(
            ...
            fake_emails=[
                {
                    ...
                    "cc_addresses": "[]",
                    "subject": "Test",
                    "body_preview": "SENSITIVE EMAIL BODY",
        ...
        assert entries[0]["body_preview"] == ""

    def test_body_preview_preserved_when_enabled(self):
        """body_preview is kept when content_snippets is true."""
        entries = self._run_adapter_capture_entries(
            ...
            fake_emails=[
                {
                    ...
                    "cc_addresses": "[]",
                    "subject": "Test",
                    "body_preview": "Email body content here",
                    "received_at": "2026-01-01T00:00:00",
                    "labels": "[]",
                    "has_attachments": False,
                    "is_read": True,
        ...
        assert len(entries) == 1
        assert entries[0]["body_preview"] == "Email body content here"
