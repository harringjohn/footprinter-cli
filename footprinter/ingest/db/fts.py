...
def _fts_trigger_sql(fts_table: str) -> list[str]:
    ...
    new_vals = ", ".join(_fts_col_expr(c, "new", content_cols) for c in cols)
    old_vals = ", ".join(_fts_col_expr(c, "old", content_cols) for c in cols)
...
def check_fts_triggers(conn) -> list[str]:
    ...
    return [name for name in _FTS_TRIGGER_NAMES if name not in present]


def drop_fts_triggers(conn) -> None:
    ...
    try:
        ...
        for fts_table in _FTS_DEFINITIONS:
            if fts_table in fts_tables:
                for sql in _fts_trigger_sql(fts_table):
                    cursor.execute(sql)
...
def rebuild_fts_indexes(conn) -> None:
    ...
    try:
        ...
        for fts_table in _FTS_DEFINITIONS:
            ...
            cursor.execute(_fts_create_sql(fts_table))
            cursor.execute(_fts_backfill_sql(fts_table))
...
def check_fts_health(conn) -> dict:
    ...
    cursor = conn.cursor()
    result = {}
    all_missing = set(check_fts_triggers(conn))

    for fts_table, base_table in _FTS_TABLE_MAP.items():
...
def repair_fts(conn) -> dict:
    ...
    try:
        ...
        before = {}
        for fts_table in _FTS_TABLE_MAP:
            try:
...
def init_fts_tables(conn) -> None:
    """Create FTS5 tables, triggers, and backfill indexes.

    Called from init_schema() after all base tables are created.
    """
    cursor = conn.cursor()
    ...
    for fts_table in _FTS_DEFINITIONS:
        try:
            cursor.execute(_fts_create_sql(fts_table, if_not_exists=True))
        except sqlite3.OperationalError as e:
            if "no such module: fts5" in str(e):
                logger.warning(
