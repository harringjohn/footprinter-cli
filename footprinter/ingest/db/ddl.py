...
def init_schema(conn):
    ...
    _migrate_project_name_column(conn)
    _ensure_super_entity_columns(conn)
...
def _migrate_access_columns(conn):
    ...
    try:
        for trigger_name in fts._FTS_TRIGGER_NAMES:
            conn.execute(f"DROP TRIGGER IF EXISTS {trigger_name}")

        for table in ACCESS_CONTROL_TABLES:
            conn.execute(f"ALTER TABLE {table} RENAME COLUMN mcp_view TO visibility")
            conn.execute(f"ALTER TABLE {table} RENAME COLUMN mcp_read TO access")
            try:
                conn.execute(f"ALTER TABLE {table} RENAME COLUMN mcp_view_source TO visibility_source")
                conn.execute(f"ALTER TABLE {table} RENAME COLUMN mcp_read_source TO access_source")
        ...
        for table in ACCESS_CONTROL_TABLES:
            conn.execute(f"UPDATE {table} SET visibility = 'full' WHERE visibility = 'visible'")
        conn.execute("UPDATE visibility_policies SET setting = 'full' WHERE setting = 'visible'")
...
def _migrate_project_name_column(conn):
    ...
    conn.execute("BEGIN IMMEDIATE")
    try:
        conn.execute("ALTER TABLE projects RENAME COLUMN project_name TO name")
        result = conn.execute("PRAGMA integrity_check").fetchone()
        if result[0] != "ok":
            raise RuntimeError(
...
def _ensure_source_columns(conn):
...
def _ensure_vectorize_column(conn):
    """Add vectorize column and backfill from JSON metadata (idempotent)."""
    for table in _VECTORIZE_TABLES:
        try:
            conn.execute(
...
def _ensure_updated_at_columns(conn):
    ...
    for table in _UPDATED_AT_TABLES:
        try:
            conn.execute(f"ALTER TABLE {table} ADD COLUMN updated_at DATETIME")
        except sqlite3.OperationalError as e:
            msg = str(e).lower()
            if "duplicate column" not in msg and "no such table" not in msg:
                raise
