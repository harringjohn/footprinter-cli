"""Shared access-policy helpers used by ``setup.py`` and ``permission_cmd.py``
(``fp permission check/set/reset``).
"""
...
def check_file_path(conn: sqlite3.Connection, path: str, json_output: bool, verbose: bool = False) -> int:
    ...
    if json_output:
        ...
        if verbose:
            console.print(
                "  [dim](--verbose adds no additional detail — single-file output "
